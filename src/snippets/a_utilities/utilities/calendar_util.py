...
import arrow
from utilities.list_util import chunk, chunks
...
def get_padded_dates_in_range(start_date: date, end_date: date) -> List[date]:
    time_default = time(0, 0)
    padded_start = pad_to_beginning_of_week(start_date)
    padded_end = pad_to_end_of_week(end_date)
    date_range = arrow.Arrow.range(
        "day",
        datetime.combine(padded_start, time_default),
        datetime.combine(padded_end, time_default),
    )
    date_range = [date(x.year, x.month, x.day) for x in date_range]
    return date_range
